...
class JugadoresModel extends BaseModel {
  ...
  function agregarJugador($jugador){
    ...
    function getJugadoresEquipo($id_equipo){
      ...
      $consulta = $this->db->prepare("SELECT * FROM Jugadores INNER JOIN Equipos ON Jugadores.fk_id_equipo = Equipos.id INNER JOIN Posiciones ON Jugadores.posicion = Posiciones.rk_id_posicion  INNER JOIN Posiciones ON Jugadores.foto = Imagenes.id WHERE fk_id_equipo=$id_equipo");
      $consulta->execute();
      return $consulta->fetchAll();
    }

    function GetAll(){
      $consulta = $this->db->prepare("SELECT * FROM Jugadores INNER JOIN Equipos ON Jugadores.fk_id_equipo = Equipos.id INNER JOIN Posiciones ON Jugadores.posicion = Posiciones.rk_id_posicion INNER JOIN Posiciones ON Jugadores.foto = Imagenes.id");
      $consulta->execute();
      return $consulta->fetchAll();
    }
}
